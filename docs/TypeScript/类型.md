...
null 和 undefined 可以赋给任何的类型，因为它们是所有其他类型的一个有效值。配置中开启 `--strictNullChecks`标记后，声明一个变量就不会自动包含 null 或 undefined。开启了 `--strictNullChecks`后，可选参数/属性就会被自动地加上 | undefined。

## 交叉类型

T & U。将多个类型合并为一个类型，相当于新类型具有这多个类型的所有特性，通常在使用混入（mixin）的场合使用交叉类型。
...
T | U。限制传入的值的类型只能是 | 分隔的每个类型。如果一个值是联合类型，那么只能访问它们中共有的属性与方法。
...
- `in` 则可以遍历枚举类型。keyof 产生联合类型, in 则可以遍历枚举类型。
...
使每个属性都只读，但只能作用到一级对象的属性，对嵌套的对象属性不起作用。
...
- 如果 `T extends U ? X : Y`, `T` 可能是 `A | B` 的联合类型, 那实际情况就变成 `(A extends U ? X : Y) | (B extends U ? X : Y)`。
