...
DOM 型 XSS 主要和前端 JS 有关，是前端 JS 获取到用户的输入没有进行过滤就拼接到 HTML 中，从而通过 DOM 来动态修改页面内容。
...
2. 对用户输入的内容进行转义。比如将 `<`、`>` 等字符转义为[字符实体](https://www.w3school.com.cn/html/html_entities.asp)。但对于富文本信息的话，可能会一刀切掉所有的标签，所以需要再设置黑名单或白名单，只对某些特殊的标签进行转义。
...
跨站请求伪造。攻击者伪造请求，冒充用户在站内进行操作。因为大多数网站是通过 Cookie 等方式识别用户身份的（包括 Session，SessionID 也基本是保存在 Cookie 里），所以攻击者可以通过欺骗用户点击某个链接，带上该网站的 Cookie 进行某些非法操作。而因为请求信息里有了 Cookie，所以第三方网站不会知道这是一次 CSRF 攻击而通过这次非法请求，并且这次攻击对用户来说可能是无知觉的。（需要依赖于用户点击）
...
1. 验证请求头的 Referer 字段（记录了该 HTTP 请求的来源地址），判断请求来源是否合法。存在的问题是，CSRF 防御完全由浏览器的 Referer 字段来保证，并不能完全保证安全，因为在某些浏览器下 Referer 是可以伪造的。

2. 在请求地址中添加 token 验证。CSRF 攻击之所以能够成功，是因为黑客可以完全伪造用户的请求，该请求中所有的用户验证信息都是存在于 Cookie 中，因此黑客可以在不知道这些验证信息的情况下直接利用用户自己的 Cookie 来通过安全验证。要抵御 CSRF，关键在于在请求中放入黑客所不能伪造的信息，并且该信息不存在于 Cookie 之中。可以在 HTTP 请求中以参数的形式加入一个随机产生的 token，并在服务器端建立一个拦截器来验证这个 token，如果请求中没有 token 或者 token 内容不正确，则认为可能是 CSRF 攻击而拒绝该请求。

3. 验证码。强制用户进行某些操作时，必须通过验证码验证合法性后才能完成最终请求。但这对于用户体验不是很好。

4. 在请求头中加入一个自定义的字段类似于 Token，用来给服务端判断该请求是否合法。这样攻击者就无法获取到这个自定义的 Token 而无法成功发起 CSRF 了。

5. Cookie设置 SameSite，使 Cookie 不随跨域请求发送。